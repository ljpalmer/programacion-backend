...
CART.delete('/carts/:cid', async (request , response) => {
    ...
    //   console.log(products);      
      try {
        ...
        const result = await cartModel.findByIdAndUpdate( cartId, { $pull: { products: { type: { $in: productIds } } } }, { new: true } );  
...
CART.put('/:cid', async (request, response) => {
    ...
    try {
        ...
        productsToUpdate.forEach(async element => {
            await cartModel.findOneAndUpdate({_id: cartId, 'products.type': element._id}, { $set: { 'products.$.quantity': element.quantity }});    
        });
...
CART.post('/', async (request, response) => {                 
     try {        
        ...
        let data = new cartModel(itemCart);
...
CART.get('/:cid', async (request, response) => {
    ...
    let cart = cartModel.findById(id);
